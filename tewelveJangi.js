...
GAME.prototype.init_game = function () {
    ...
    board[0][0] = new MAL(PLAYER2, Jang, new POS(BOARD, 0, 0), false);
    board[0][1] = new MAL(PLYAER2, Wang, new POS(BOARD, 0, 1), false);
    board[0][2] = new MAL(PLYAER2, Wang, new POS(BOARD, 0, 2), false);
    board[1][0] = new MAL(NOTHING, Mu, new POS(BOARD, 1, 0), false);
    board[1][1] = new MAL(PLYAER2, Ja, new POS(BOARD, 1, 1), false);
    board[1][2] = new MAL(NOTHING, Mu, new POS(BOARD, 1, 2), false);
    board[2][0] = new MAL(NOTHING, Mu, new POS(BOARD, 2, 0), false);
    board[2][1] = new MAL(PLYAER1, Ja, new POS(BOARD, 2, 1), false);
    board[2][2] = new MAL(NOHTING, Mu, new POS(BOARD, 2, 2), false);
    board[3][0] = new MAL(PLAYER1, Sang, new POS(BOARD, 3, 0), false);
    board[3][1] = new MAL(PLYAER1, Wang, new POS(BOARD, 3, 1), false);
    board[3][2] = new MAL(PLYAER1, Jang, new POS(BOARD, 3, 2), false);
}
...
GAME.prototype.get_having = function (player) {
    ...
    if (player == PLAYER2) return this.palyer2;
    return null;
}
...
GAME.prototype.button_click = function (player, pos) {
    ...
    if (pos.from == HAVING) {
            ...
            for (let i = 1; i < 4; i++) {
                for (let j = 0; j < 3; j++) {
                    if (this.board[i][j].type == Mu) {
                        this.boad[i][j].highlighted = true;
                    }
                }
            }
        }
        ...
        if (this.board[pos.y][pos.x].type.highlighted) {
            if (this.board[pos.y][pos.x].type == Wang) {
                this.state = player;
            }
            else if (this.board[pos.y][pos.x].type != Mu) {
                let this_player = (player == PLYAER1 ? player1 : player2);
                for (let i = 0; i < 6; i++) {
            ...
            this.board[this.selected.y][this.selected.x] = new MAL(NOTHING, Mu, new POS(BOARD, this.selected.y, this.selected.x), false);
            ...
            if(player == PLAYER1) {
                ...
                for(let i=0; i<3; i++) {
                    if(this.board[0][i].type == Wang && this.board[0][i].player == PLAYER2) {
                        this.state = PLAYER1;
                    }
                }
            }
            ...
            for (let i = 0; i < MAL_DIR[this.selected.type].length; i++) {
                let y = pos.y + (player == PLYAER1 ? 1 : -1) * MAL_DIR[this.selected.type][0];
                let x = pos.x + MAL_DIR[this.selected.type][1];
